...
for(i in names){
  ...
  merged_count[[base_name]] <- filecount
  
}

## Get % mapped reads in D1 ##
merged_count_29 <- lapply(merged_count, function(df) {
  df$count <- as.numeric(df$count)
  return(df)
})

final_count_29 <- merged_count_29 %>%
  bind_rows() %>%                  # Combine data frames
  group_by(ID) %>%                 # Group by "ID"
  summarize("count" = sum(count, na.rm = TRUE))

## Get most abundant genes in D1 ##
...
## Get % mapped reads in D3 ##
merged_count_33 <- lapply(merged_count, function(df) {
  df$count <- as.numeric(df$count)
  return(df)
})

final_count_33 <- merged_count_33 %>%
  bind_rows() %>%                  # Combine data frames
  group_by(ID) %>%                 # Group by "ID"
  summarize("count" = sum(count, na.rm = TRUE))
